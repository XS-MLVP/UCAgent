...
```bash
#!/bin/bash
# conditional_batch.sh

DUTS=("Adder" "Mux" "FSM")
CRITICAL=("Adder" "FSM")  # 关键模块，失败则停止

for dut in "${DUTS[@]}"; do
    echo "验证 $dut..."

    if ! ucagent output/ $dut -s -l -eoc --no-embed-tools; then
        # 检查是否是关键模块
        if [[ " ${CRITICAL[*]} " =~ " ${dut} " ]]; then
            echo "✗ 关键模块 $dut 验证失败，停止批处理"
            exit 1
        else
            echo "⚠ $dut 验证失败，但继续执行"
        fi
...
# 第一阶段：并行执行基础模块
echo "阶段 1：基础模块并行验证"
(ucagent output/A Adder -s -l -eoc --no-embed-tools) &
(ucagent output/B Mux -s -l -eoc --no-embed-tools) &
wait

# 第二阶段：串行执行复杂模块
echo "阶段 2：复杂模块串行验证"
ucagent output/C ALU -s -l -eoc --no-embed-tools
ucagent output/D CPU -s -l -eoc --no-embed-tools
```
...
for dut in Adder Mux FSM; do
    echo "验证 $dut（超时: ${TIMEOUT}秒）..."

    if timeout $TIMEOUT ucagent output/ $dut -s -l -eoc --no-embed-tools; then
        echo "✓ $dut 验证成功"
    else
        status=$?
...
for dut in "${DUTS[@]}"; do
    log_file="$LOG_DIR/${dut}.log"
    echo "验证 $dut，日志: $log_file"

    {
        ...
        if ucagent output/ $dut -s -l -eoc --no-embed-tools 2>&1; then
            echo "状态: 成功"
        else
            echo "状态: 失败（退出码: $?）"
        fi
...
echo "生成汇总报告..."
{
    ...
    for dut in "${DUTS[@]}"; do
        log_file="$LOG_DIR/${dut}.log"
        status=$(grep "状态:" $log_file | cut -d: -f2)
        echo "- $dut: $status"
    done
} > $LOG_DIR/summary.txt
...
verify_with_retry() {
    ...
    while [ $retries -lt $MAX_RETRIES ]; do
        echo "验证 $dut（尝试 $((retries+1))/$MAX_RETRIES）..."
    ...
    echo "✗ $dut 验证失败（已重试 $MAX_RETRIES 次）"
    return 1
}
...
send_notification() {
    ...
    if command -v mail &> /dev/null; then
        echo "$message" | mail -s "$title" user@example.com
    fi

    # 企业微信/钉钉 Webhook（示例）
    # curl -X POST "https://qyapi.weixin.qq.com/..." -d "{\"text\":\"$message\"}"
}

# 执行批处理
if make api_batch; then
    send_notification "UCAgent 批处理成功" "所有验证任务已完成"
else
    send_notification "UCAgent 批处理失败" "部分验证任务失败，请检查日志"
fi
```
...
echo "检测到 $CORES 个核心，设置并行数为 $MAX_PARALLEL"

# 使用 GNU parallel 控制并发
parallel -j $MAX_PARALLEL ucagent output/{} {} -s -l -eoc --no-embed-tools ::: Adder Mux FSM ALU
```
...
verify_dut() {
    local dut=$1
    local log_file="logs/${dut}_$(date +%Y%m%d_%H%M%S).log"

    {
        ucagent output/ $dut -s -l -eoc --no-embed-tools 2>&1
    } | tee $log_file

    local status=${PIPESTATUS[0]}

    if [ $status -ne 0 ]; then
        echo "错误：$dut 验证失败（退出码: $status）" >&2
...
if [ ! -f $STATE_FILE ]; then
    echo '{"completed": [], "failed": [], "pending": ["Adder", "Mux", "FSM"]}' > $STATE_FILE
fi

# 获取待处理列表
PENDING=$(jq -r '.pending[]' $STATE_FILE)

for dut in $PENDING; do
    if ucagent output/ $dut -s -l -eoc --no-embed-tools; then
        # 标记为完成
        jq ".completed += [\"$dut\"] | .pending -= [\"$dut\"]" $STATE_FILE > tmp.$$ && mv tmp.$$ $STATE_FILE
    else
        # 标记为失败
        jq ".failed += [\"$dut\"] | .pending -= [\"$dut\"]" $STATE_FILE > tmp.$$ && mv tmp.$$ $STATE_FILE
    fi
done
...
# 添加定时任务（每天凌晨 2 点执行）
0 2 * * * cd /path/to/UCAgent/examples/BatchRun && make api_batch >> /var/log/ucagent_batch.log 2>&1
```

**使用 systemd timer**：

```ini
# /etc/systemd/system/ucagent-batch.service
[Unit]
Description=UCAgent Batch Verification
After=network.target

[Service]
Type=oneshot
User=youruser
WorkingDirectory=/path/to/UCAgent/examples/BatchRun
ExecStart=/usr/bin/make api_batch
...
```ini
# /etc/systemd/system/ucagent-batch.timer
[Unit]
Description=Run UCAgent Batch Verification Daily
...
```bash
sudo systemctl enable ucagent-batch.timer
sudo systemctl start ucagent-batch.timer
```
