...
UCAgent 由四大核心组件构成，它们相互协作完成验证任务：
...
**作用**：提供 LLM 推理能力，执行实际的验证任务。

**支持的后端类型**：

- `langchain`：通过 LangChain 框架接入各种 LLM（OpenAI、Claude、本地模型等）
- `claude`：直接使用 Anthropic Claude 命令行工具
- `opencode`：使用 OpenAI Codex
...
```yaml
backend:
  key_name: "langchain" # 使用 langchain 后端
  langchain:
    clss: ucagent.abackend.langchain.UCAgentLangChainBackend
```

> 💡 **提示**：本教程的 MiniWorkflow 示例使用 **MCP 模式**，通过外部 Code Agent（如 Claude、Cursor）执行任务，**不需要配置 backend**。

### 2. Stage Manager（阶段管理器）
...
**下一步**：现在您已经了解了 UCAgent 的"骨骼"和"血液循环"，接下来让我们学习如何定义工作流配置，也就是为系统注入"灵魂"！

👉 继续阅读：[创建工作流配置](02_create_workflow.md)
