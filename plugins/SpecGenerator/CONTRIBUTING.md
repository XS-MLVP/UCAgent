...
- 日常变更限于本插件目录，包括依赖、测试和 CI 步骤。通用贡献流程和漏洞报告渠道使用 UCAgent 的约定；插件专用的 `.gitignore`、`.editorconfig`、`.gitattributes` 和安全边界说明在本目录维护。
...
UCAgent 根目录的 `.github/workflows/spec-generator.yml` 负责触发、Linux/macOS 环境和安装当前核心，随后调用本目录的 [ci/action.yml](ci/action.yml)。修改插件的安装、检查或打包步骤只需修改该 Action 或 Makefile；依赖同步修改本目录的 `pyproject.toml` 和插件声明。
