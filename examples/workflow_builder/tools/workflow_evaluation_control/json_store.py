...
def document_record_template(relative: str) -> dict[str, Any]:
    ...
    if relative in REPORT_FILES or relative == "eval/incremental_report.json":
        ...
        if report_type == "checkers":
            template["record"]["checks"][0]["evidence"].append(
                {
                    "kind": "producer_checker",
                    "producer": "registered producer tool or documented writer",
                    "checker": "bound Checker class",
                    "artifact": "tmp/path/to/real/producer/artifact",
                    "observation": "The real producer artifact and the Checker's actual result.",
                }
            )
            template["finding_template"]["evidence"].append(
                {
                    "kind": "reachability",
                    "producer": "registered producer, user boundary, or partial-write path",
                    "artifact": "runtime artifact path and field",
                    "reachable": True,
                    "observation": "Why the tested state can occur in the bound runtime data flow.",
                }
            )
            template["note"] += (
                " Checker findings based on fixtures or malformed fields require reachability evidence; "
                "unreachable synthetic cases must be info/suspected."
            )
        return template
    if relative == "eval/user_suggestions.json":
        return {
