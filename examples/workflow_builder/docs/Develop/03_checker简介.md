...
本篇说明如何修改母工作流的 Checker 规划、生成、模板、fixture 和注册系统，使后续生成的业务工作流拥有正确 Checker。读者是维护 `workflow_checker_generator/`、Builder 中心 spec 和母阶段验收的开发者，不是被 Checker检查的子 Agent。
...
- `workflow_checker_generator/core.py`：spec、源码、fixture、注册和报告。
- `workflow_checker_generator/templates.py`：通用规则 renderer。
- `workflow_builder/core.py`：从中心 workflow spec 直接物化业务 Checker。
...
Checker 的中心规划位于 `workflow_build.yaml` 的 workflow spec。构建阶段应直接从该设计生成 Checker 源码、fixture 和注册，避免等到后期再猜每个阶段需要什么检查。相关实现主要位于 `workflow_checker_generator/`、`workflow_builder/core.py` 和 `workflow_evaluation_control/uc_checkers.py`。
...
源码位置：`workflow_checker_generator/core.py`，符号 `_validate_spec()`。
...
源码位置：`workflow_checker_generator/core.py`，符号 `_validate_inline_source()`。
...
源码位置：`workflow_checker_generator/core.py`，符号 `_materialize_auto_tests()`。
...
源码位置：`workflow_checker_generator/templates.py`，符号 `_render_json_required_keys()`。
...
源码位置：`workflow_checker_generator/core.py`，符号 `_register_checker()`。
...
源码位置：`workflow_checker_generator/core.py`，符号 `generate_checkers_from_specs()`。
...
以下类定义在 `workflow_builder/uc_checkers.py`。大部分类继承 `WorkflowBuildConfigChecker`，共享 workspace 路径规范化、YAML/JSON 读取和证据结构，但每个类的业务证明对象不同。
...
以下类定义在 `workflow_evaluation_control/uc_checkers.py`。它们不判断某个业务文件“好不好”，而是判断评估/增量报告是否按契约收集证据、是否受批准约束以及是否真正完成部署回归。
