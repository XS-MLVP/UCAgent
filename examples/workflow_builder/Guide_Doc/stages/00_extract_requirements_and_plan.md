...
3. 第1.2步：{DESC_FILE} 是唯一需求来源；本文件只是只读流程指导，不能作为业务需求或覆盖证据。另有动态只读输入目录 {TEST_INPUT_DIR}/。禁止读取 wfgen/guide.md、tools/workflow_builder/uc_checkers.py、Guide_Doc/stage_check_guide.md、workflow_build_schema.yaml 或 workflow_build_yaml_guide.md；构建配置的 schema 和模板规则由下一阶段负责，避免需求规划与实现细节互相污染。
