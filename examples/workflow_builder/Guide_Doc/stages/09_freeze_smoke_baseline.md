# Stage 09: freeze_smoke_baseline

## 阶段目标

冻结已通过 smoke 的初步模板基线。本文件是维护者根据历史运行经验维护的静态指导，不由运行中的 Agent 修改。执行阶段时必须先读取本文件，再处理业务产物。
...
1. 读取 {WFGEN_DIR}/requirements_manifest.yaml 和前序阶段生成的 mcp_baseline_evidence.yaml，核对其中的工具清单、direct/MCP 结果、日志路径与失败摘要，确认代表性工具、checker 和子 UCAgent 均真实通过。
2. 将 manifest.milestones.smoke_ready 更新为 true；feature_complete 和 release_ready 必须保持 false。
3. 记录 smoke 基线中可复用的启动方式、公共工具协议、checker 协议和已知限制，然后 Complete。
...
一次正常执行会先读取本指导、全部业务 reference 和 `wfgen/workflow_implementation_plan.md`，完成 `freeze_smoke_baseline` 的真实工作，读取 Checker 或测试输出确认结论，然后调用 `WorkflowPlanAppender` 追加本阶段的决策、产物、问题和后续约束。只有规划追加成功且配置中的 Checker 全部通过，才调用 `SetCurrentStageJournal` 和 `Complete`。

## Checker 与通过条件

- `smoke_manifest_check`：必须读取真实 artifact 并满足配置及本指导规定的结构化契约。
