...
test_%: init_%
	python3 ucagent.py $(CWD)/ $* --config $(CFG) -s -hm --tui -l --use-skill --no-embed-tools ${ARGS}

mcp_%: init_%
	python3 ucagent.py $(CWD)/ $* --config $(CFG) -s -hm --tui --use-skill --mcp-server-no-file-tools --no-embed-tools ${ARGS}
