...
使用 `RunSkillScript` 工具执行以下命令，生成 checker/wrapper 骨架：

```bash
python3 .ucagent/skills/env-generation/scripts/generate_formal_env.py -dut_name {DUT}
```

说明：通过 `RunSkillScript` 执行时，脚本会自动复用环境变量 `DUT` 和 `OUT` 推导路径；通常无需传 `-rtl_dir`。
