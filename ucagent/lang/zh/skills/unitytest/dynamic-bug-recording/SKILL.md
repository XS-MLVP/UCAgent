...
同一BG的后续失败TC直接调用`ApplyWaveInfoEvidence`，不要复制BG；工具从目标测试函数docstring读取中文可见标题，并把新TC插入首个分析标题之前，因此测试源码和docstring必须先存在。技能未启用或脚本不可用时，使用文本编辑工具按相同来源填写具体中文名称，并复现Guide_Doc/dut_bug_analysis.md中的第 5.1 节层级与结构；任务要求和完成标准不变。
