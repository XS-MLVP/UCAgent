...
本技能可调用两个可选的批量脚本；没有`RunSkillScript`时，所有步骤都可以使用内置文本编辑工具完成:
- `unitytest/dynamic-bug-recording`中的`record_dynamic_bug.py`: 将新证实的动态Bug按规范写入`{OUT}/{DUT}_bug_analysis.md`
- 本技能中的`linkbug.py`: 根据`<STATIC-BUG-SUMMARY>`、`<STATIC-BUG-DETAILS>`、`<STATIC-BUG-PROGRESS>`固定分区标记，将静态Bug与动态Bug的关联关系回填到`{OUT}/{DUT}_static_bug_analysis.md`；脚本不解析中文标题，旧的仅标题格式不兼容

这两个可选脚本分工如下:
- `record_dynamic_bug.py`负责创建或更新动态Bug记录
- `linkbug.py`负责把静态Bug条目下的`<LINK-BUG-[BG-TBD]>`替换成最终结论

也就是说:
- 若当前静态Bug已经有可直接复用的动态Bug记录，可以直接用文本编辑工具同步回填两处LINK；`linkbug.py`可用时也可执行相同操作
- 若当前静态Bug还没有动态Bug记录，默认使用文本编辑工具按Guide第6.1.1节最小骨架示例建立结构；`record_dynamic_bug.py`可用时可改用它生成相同骨架。随后用`ApplyWaveInfoEvidence`写入最终receipt的机器字段并由LLM填完语义分析，最后用文本编辑工具或可选`linkbug.py`同步回填LINK
...
- `record_dynamic_bug.py`只生成带`<BUG-TODO>`的动态Bug骨架，不做根因判断，也不生成真实波形字段。最终WaveInfo调用后必须使用`ApplyWaveInfoEvidence`写入机器证据，再由LLM读取失败日志、timeline和RTL，填完该BG内的三个语义结论及全部分析标记字段
...
当Bug已证实时，先按Guide第6.1.1节使用文本编辑工具生成骨架；若共享技能`unitytest/dynamic-bug-recording`可用，也可以通过`RunSkillScript`执行`record_dynamic_bug.py`生成相同骨架：

```text
["unitytest/dynamic-bug-recording", "record_dynamic_bug.py", "-BG 'BG-ADD-SPECIAL-VALUE-90' -TC 'TC-tests/test_ALU754_arithmetic.py::test_add_special' -BD '+INF 与 -INF 相加时未返回 NaN，而是错误输出 0x00000000。'"]
```
...
注意:
- `record_dynamic_bug.py`只把未完成骨架写入`{OUT}/{DUT}_bug_analysis.md`
...
- 6.**先记动态Bug,再回填静态Bug**: 新发现的动态Bug必须先通过文本编辑工具或可选`record_dynamic_bug.py`写入并完成`bug_analysis.md`,再通过文本编辑工具或可选`linkbug.py`建立关联
- 7.**两阶段写入**: 新增BG/TC结构可通过文本编辑工具按Guide骨架示例完成，也可使用可选`record_dynamic_bug.py`；随后必须用`ApplyWaveInfoEvidence`写入波形机器字段，再用文本编辑工具填完该BG的语义分析模板。`static_bug_analysis.md`中的链接可直接同步编辑，`linkbug.py`只是可选助手
...
1. 允许一次输入多条同类命令：可以批量执行若干`record_dynamic_bug.py`生成骨架；填充模板后，再单独批量执行对应的`linkbug.py`。不能在同一次`RunSkillScript`调用中跳过LLM填空而从record直接link
...
5. 若本轮需要新建动态Bug记录，使用文本编辑工具或可选脚本时都必须保持以下顺序:
   - 先创建动态Bug骨架（直接编辑或执行`record_dynamic_bug.py`）
...
```text
["unitytest/dynamic-bug-recording", "record_dynamic_bug.py", "-BG 'BG-MUL-OVERFLOW-THRESHOLD-85' -TC 'TC-tests/test_ALU754_arithmetic.py::test_mul_overflow' -BD '最大规格化数乘以 2.0 时未拉高 overflow，也未输出 +INF。'"]
# 填完该BG的WaveInfo证据和全部分析章节后，才能执行下一条linkbug.py
["unitytest/static-bug-validation", "linkbug.py", "-SBG 'BG-STATIC-007-OVERFLOW-THRESHOLD' -LBG 'BG-MUL-OVERFLOW-THRESHOLD-85'"]
```
...
```text
["unitytest/dynamic-bug-recording", "record_dynamic_bug.py", "-BG 'BG-FSM-DEAD-92' -TC 'TC-tests/test_demo.py::test_static_demo_1' -BD '第一个动态Bug描述'"]
["unitytest/dynamic-bug-recording", "record_dynamic_bug.py", "-BG 'BG-FSM-DEFAULT-85' -TC 'TC-tests/test_demo.py::test_static_demo_2' -BD '第二个动态Bug描述'"]
# 分别填完两个BG的WaveInfo证据和全部分析章节后，再建立静态链接
["unitytest/static-bug-validation", "linkbug.py", "-SBG 'BG-STATIC-020-FSM-ISSUE' -LBG 'BG-FSM-DEAD-92,BG-FSM-DEFAULT-85'"]
```
