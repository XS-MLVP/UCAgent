...
def get_chat_model_openai(model,cfg: Config, callbacks, rate_limiter) -> Any:
    """Get OpenAI chat model instance.

    Args:
        model: Configuration containing OpenAI settings.
...
def get_chat_model_anthropic(model,cfg: Config, callbacks, rate_limiter) -> Any:
...
def get_chat_model_google_genai(model,cfg: Config, callbacks, rate_limiter) -> Any:
    try:
...
def get_chat_model(model_cfg, cfg: Config, callbacks: Any = None) -> Any:
    ...
    model_type= model_cfg.get_value("model_type", "openai")
    func = "get_chat_model_%s" % model_type
    echo_g(f"Using model type: {model_type} in model.")
    if func in globals():
        return globals()[func](model_cfg.get_value(model_type), cfg, callbacks, rate_limiter)
    else:
        raise ValueError(
