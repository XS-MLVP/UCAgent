...
def description_bug_doc():
    ...
    return [
        ...
        f"  - The first non-empty content after every TC must start with {WAVEFORM_FENCE_OPEN} and contain a mapping whose only top-level key is {WAVEFORM_BLOCK_KEY}. After a final WaveInfo call, use ApplyWaveInfoEvidence to write the generated mapping and viewer link for that exact BG/TC. Then add non-empty {waveform_analysis_fields} from the returned timeline and RTL. Do not copy, invent, or edit receipt-backed fields.",
        "  - ApplyWaveInfoEvidence owns exactly one BG/TC pair per call and preserves every other pair. For multiple failed TCs with one root cause, reuse the BG and call each TC separately. If one failed TC exposes independent Bugs, keep distinct BGs and call the same TC separately for each BG; reuse one receipt only when its signed window and signal_groups support every target Bug. Cross-BG application does not require replace_existing.",
        ...
        "  - The first non-empty content after that YAML fence must be the same final WaveInfo result's <WAVEFORM-VIEWER> tagged Markdown link written by ApplyWaveInfoEvidence. Its marker, /surfer/?wave= route, and signed token must not be edited or constructed manually.",
