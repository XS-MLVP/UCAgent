...
def description_bug_doc():
    ...
    return [
        ...
        "  - Use this semantic heading hierarchy: '### <group title> <FG-GROUP>' -> '#### <function title> <FC-FUNCTION>' -> '##### <checkpoint title> <CK-CHECKPOINT>' -> '###### <bug title>\uff08XX%\uff09 <BG-BUGNAME-XX>' -> '- <test title> <TC-WORKSPACE_RELATIVE_TEST::[ClassName::]test_name>'. Angle-bracket tags may be hidden by Markdown, so every visible title must describe the actual item rather than repeat its type.",
